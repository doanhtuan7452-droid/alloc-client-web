...
export default function TaskCard({ 
  ...
  return (
    <div className={`bg-black border ${isActive ? 'border-cyan-500/50 shadow-[0_0_15px_rgba(34,211,238,0.1)]' : 'border-border-default'} rounded-lg p-4 relative overflow-hidden flex flex-col ${opacity < 100 ? 'opacity-75' : ''}`}>
      {isActive && <div className="absolute top-0 left-0 w-1 h-full bg-cyan-400"></div>}
      
      {tags.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 mb-3">
          {tags.map((tag, idx) => (
            <span key={idx} className={`${tag.bgClass || 'bg-neutral-800'} ${tag.textClass || 'text-neutral-300'} border ${tag.borderClass || 'border-border-subtle'} text-[10px] uppercase font-mono px-2 py-0.5 rounded`}>
              {tag.label}
            </span>
          ))}
        </div>
