...
export default function Team() {
    ...
    async function loadData() {
      ...
      try {
        ...
        for (const c of cycles) {
          const evals = await WorkspaceService.getEvaluations(
            workspaceId,
            c.cycleID,
          );
  ...
  const fetchProfile = async (member) => {
    if (!memberProfiles[member.workspaceMemberId]) {
      const p = await WorkspaceService.getMemberProfile(
        workspaceId,
    ...
    try {
      ...
      if (type === "Leave") {
        setLeaveRequests((prev) =>
          prev.map((r) => (r.requestId === id ? updatedReq : r)),
        );
      } else {
        setOtRequests((prev) =>
          prev.map((r) => (r.requestId === id ? updatedReq : r)),
      ...
      alert(
        `${action === "approve" ? "Phê duyệt" : "Từ chối"} đơn ${type} thành công!`,
      );
    } catch (e) {
      alert("Lỗi: " + e.message);
    }
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-content-muted text-sm">
              Quản lý thành viên, xem hồ sơ kỹ năng, duyệt đơn từ (Leave/OT) và
              theo dõi đợt Performance Review.
            </p>
          </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-full bg-inset overflow-hidden border-2 border-border-default flex items-center justify-center font-mono font-bold text-xs text-content-muted shrink-0">
                        {member.resource.avatarUrl ? (
                          <img
                            src={member.resource.avatarUrl}
                            alt={member.resource.fullName}
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="font-medium text-content-primary text-sm line-clamp-1">
                            {member.resource.fullName}
                          </h4>
              ...
              <div className="space-y-3 max-h-[220px] overflow-y-auto pr-1 custom-scrollbar">
                {isLoading ? (
                  ...
                  pendingRequests.map((req) => (
                    <div
                      key={req.requestId + req.type}
                      className="bg-surface/40 border border-white/5 p-3 rounded-lg hover:border-border-default transition-colors"
                    >
                      <div className="flex justify-between items-start mb-1">
                        ...
                        <span
                          className={`text-[9px] font-mono px-1.5 rounded ${req.type === "Leave" ? "bg-purple-950 text-purple-400 border border-purple-800/30" : "bg-blue-950 text-blue-400 border border-blue-800/30"}`}
                        >
                          {req.type}
                        </span>
                      </div>
                      ...
                      {req.type === "Leave" ? (
                        <p className="text-[11px] text-content-muted font-mono mb-2">
                          {req.startDate} to {req.endDate}
                        </p>
                      ) : (
                        <p className="text-[11px] text-content-muted font-mono mb-2">
                          {req.requestedDate} - {req.expectedHours}h
                        </p>
                      )}
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() =>
                            handleActionRequest(req.type, req.requestId, "deny")
                          }
            ...
            <div className="bg-white/[0.02] backdrop-blur-md border border-white/10 rounded-xl p-6 flex flex-col flex-1">
              ...
              <div className="space-y-3 flex-1 overflow-y-auto custom-scrollbar">
                {isLoading ? (
                  ...
                  <p className="text-xs text-content-muted italic py-4 text-center">
                    Không có chu kỳ đánh giá nào.
                  </p>
                ) : (
                  reviewCycles.map((cycle) => {
                    const cycleEvals = evaluations[cycle.cycleID] || [];
                    const selfEvals = cycleEvals.filter(
                      (e) => e.evaluationType === "Self",
                    ).length;
                    ...
                    return (
                      <div
                        key={cycle.cycleID}
                        className="bg-inset/40 border border-white/5 p-3 rounded-lg flex flex-col justify-between h-full"
                      >
                        <div>
                          ...
                          <div className="space-y-2 font-mono text-[11px]">
                            ...
                            <div className="flex justify-between items-center bg-white/[0.01] p-1.5 rounded border border-white/5">
                              ...
                              <span className="text-content-secondary font-bold text-[9px]">
                                {cycle.endDate}
                              </span>
                            </div>
                          </div>
                        ...
                        <button className="w-full mt-4 py-1.5 text-[11px] font-mono flex items-center justify-center gap-1 bg-white/[0.04] text-content-secondary hover:text-white border border-white/10 rounded hover:bg-white/[0.08] transition-colors cursor-pointer">
                          <TrendingUp className="w-3.5 h-3.5" /> Xem Kết Quả Chi
                          Tiết
                        </button>
                      </div>
                    );
              ...
              <div className="flex gap-4 items-center">
                <div className="w-16 h-16 rounded-full bg-inset overflow-hidden border border-white/20">
                  {selectedMember.resource.avatarUrl ? (
                    <img
                      src={selectedMember.resource.avatarUrl}
                      className="w-full h-full object-cover"
                    />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-xl font-bold bg-zinc-800 text-zinc-500">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-zinc-800/50 p-3 rounded-lg border border-white/5">
                    ...
                    <span
                      className={`text-lg font-bold font-mono ${memberProfiles[selectedMember.workspaceMemberId].performanceRating === "Outstanding" ? "text-blue-400" : "text-emerald-400"}`}
                    >
                      {
                        memberProfiles[selectedMember.workspaceMemberId]
                          .performanceRating
                ...
                <div>
                  <h4 className="font-bold text-sm mb-3 text-content-primary border-b border-white/5 pb-2">
                    Skill Overview
                  </h4>
                  <div className="space-y-3 px-1">
                    {[
                      {
                        l: "Technical Skills",
                        v: memberProfiles[selectedMember.workspaceMemberId]
                          .technicalSkillScore,
                        c: "bg-blue-500",
                      },
                      {
                      ...
                      <div key={skill.l} className="flex items-center gap-4">
                        <span className="text-xs text-zinc-400 font-mono w-32 shrink-0">
                          {skill.l}
                        </span>
                        <div className="flex-1 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                          <div
                            className={`h-full ${skill.c}`}
                            style={{ width: `${skill.v}%` }}
                          ></div>
                        </div>
                ...
                <div className="flex justify-end pt-4 border-t border-white/10">
                  <span className="text-[10px] text-zinc-500 font-mono flex items-center gap-1">
                    <Clock className="w-3 h-3" /> Last Evaluated:{" "}
                    {new Date(
                      memberProfiles[selectedMember.workspaceMemberId]
                        .lastEvaluatedAt,
                    ).toLocaleDateString()}
