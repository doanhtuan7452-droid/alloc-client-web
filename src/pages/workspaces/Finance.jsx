...
export default function Finance() {
  ...
  const loadFinancialSummary = async () => {
    if (!activeProject) return;
    try {
      const summary = await ProjectService.getFinancialSummary(
  ...
  useEffect(() => {
    ...
    async function loadExpenses() {
      ...
      try {
        const data = await ProjectService.getExpenses(activeProject.projectId, {
  ...
  useEffect(() => {
    ...
    async function loadRevenues() {
      ...
      try {
        const data = await ProjectService.getRevenues(activeProject.projectId, {
          pageNumber: revenuePage,
          pageSize: 5,
        });
        if (isSubscribed) {
          setRevenueData(data);
  ...
  useEffect(() => {
    if (isExpenseModalOpen) {
      ...
      if (
        activeProject &&
        todayStr >= activeProject.startDate &&
        todayStr <= activeProject.endDate
      ) {
  ...
  const handleAddExpense = async (e) => {
    ...
    if (
      modalDate < activeProject.startDate ||
      modalDate > activeProject.endDate
    ) {
      setModalError(
    ...
    try {
      await ProjectService.createExpense(activeProject.projectId, {
      ...
      const updatedExpenses = await ProjectService.getExpenses(
        activeProject.projectId,
        {
          pageNumber: expensePage,
          pageSize: 5,
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[50vh] text-slate-400 gap-4">
        ...
        <p className="text-sm text-slate-500 text-center max-w-sm">
          Workspace này chưa có dự án nào được khởi tạo. Vui lòng tạo dự án mới
          để hiển thị thông tin tài chính.
        </p>
      </div>
    );
  }
  ...
  const categoriesDef = [
    { name: "Personnel & Research", percent: 0.5 },
    { name: "Infrastructure (Cloud/Compute)", percent: 0.3 },
    { name: "Travel & Symposiums", percent: 0.1 },
    { name: "Software License", percent: 0.1 },
  ];
          ...
          <div className="glass-card-light rounded-xl p-6 transition-all duration-300">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-slate-400 font-mono">
                {formatCurrency(
                  remainingBudget,
                  activeProject.originalCurrencyCode,
                )}{" "}
                còn lại
              </span>
            </div>
        ...
        <div className="glass-card-light rounded-xl overflow-hidden transition-all duration-300">
          <div className="p-6 border-b border-white/5">
            <h3 className="font-bold text-base text-white">
              Phân bổ ngân sách dự án (Category Allocations)
            </h3>
            <p className="text-xs text-slate-400 mt-1">
              Cơ cấu ngân sách định mức của dự án so với thực chi.
            </p>
          </div>
          ...
          <div className="glass-card-light rounded-xl overflow-hidden flex flex-col justify-between transition-all duration-300 min-h-[360px]">
            <div>
              <div className="p-6 border-b border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h3 className="font-bold text-base text-white">
                    Sổ chi tiêu (Expenses)
                  </h3>
                  <p className="text-xs text-slate-400 mt-1">
                    Danh sách chi tiết chi phí phát sinh.
                  </p>
                </div>
              ...
              {isExpensesLoading ? (
                ...
                <div className="py-16 px-6 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-sm font-semibold text-slate-350 mb-1">
                    Chưa có chi phí nào
                  </p>
                    ...
                    <tbody className="divide-y divide-white/5 text-slate-300">
                      {expenseData.items.map((exp) => (
                        ...
                        >
                          ...
                          <td
                            className="px-6 py-3.5 font-medium text-white max-w-[140px] truncate"
            ...
            {!isExpensesLoading && expenseData.totalPages > 1 && (
              <div className="p-4 border-t border-white/5 flex items-center justify-between mt-auto">
                <span className="text-[11px] text-slate-500 font-mono">
                  Trang {expensePage} / {expenseData.totalPages} (
                  {expenseData.totalItems} chi phí)
                </span>
          ...
          <div className="glass-card-light rounded-xl overflow-hidden flex flex-col justify-between transition-all duration-300 min-h-[360px]">
            <div>
              <div className="p-6 border-b border-white/5 flex items-center justify-between">
                <div>
                  <h3 className="font-bold text-base text-white">
                    Doanh thu dự kiến (Revenues)
                  </h3>
                  <p className="text-xs text-slate-400 mt-1">
                    Kế hoạch thu tiền và đợt thanh toán của khách hàng.
                  </p>
                </div>
              </div>
                      ...
                      {revenueData.items.map((rev) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-3.5 text-center">
                            <span
                              className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded border text-[9px] font-mono ${
                                rev.status === "Received"
            ...
            {!isRevenuesLoading && revenueData.totalPages > 1 && (
              <div className="p-4 border-t border-white/5 flex items-center justify-between mt-auto">
                <span className="text-[11px] text-slate-500 font-mono">
                  Trang {revenuePage} / {revenueData.totalPages} (
                  {revenueData.totalItems} doanh thu)
                </span>
                <div className="flex gap-2">
                  <button
      ...
      {isExpenseModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div
            ...
            onClick={() => {
              if (!isSubmittingExpense) setIsExpenseModalOpen(false);
            }}
          ></div>
            ...
            <form onSubmit={handleAddExpense} className="p-6 space-y-4">
              ...
              <div>
                ...
                <span className="text-[10px] text-slate-500 mt-1 block">
                  Yêu cầu số tiền nhập phải lớn hơn 0
                </span>
              </div>
              ...
              <div>
                ...
                />
                <span className="text-[10px] text-slate-500 mt-1 block">
                  Chỉ cho phép từ {activeProject.startDate} đến{" "}
