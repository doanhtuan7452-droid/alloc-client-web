...
export default function TaskDetailModal({ task: initialTask, allTasks = [], onClose }) {
  ...
  // Kiểm tra quyền hạn
  const isOwner = currentUser?.role === "Owner" || currentUser?.role?.roleName === "Owner";
  const canEditTask = hasPermission("task:update");
  ...
  // Lọc bớt chính mình và tìm kiếm thành viên workspace
  const filteredMembers = useMemo(() => {
    return allMembers.filter((m) => {
      const isMe = m.resource?.resourceId === currentUser?.profile?.resourceId;
      if (isMe) return false;
      const displayName = m.resource?.fullName || `Member ${m.workspaceMemberId}`;
      return displayName.toLowerCase().includes(searchQuery.toLowerCase());
    });
  }, [allMembers, currentUser, searchQuery]);
  ...
  const getTaskNameById = (id) => {
    ...
    return found ? found.taskName : `Nhiệm vụ #${id}`;
  };
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col border-r border-white/10 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
              ...
              {/* HÌNH THỨC CẬP NHẬT TAB DEPENDENCIES CHỌN CÓ TÊN TASK */}
                  ...
                  {canEditTask && (
                    <form onSubmit={handleAddDependency} className="flex gap-3 items-end p-3 bg-zinc-900 border border-white/5 rounded-xl">
                      <div className="flex-1 space-y-1">
                        <label className="text-[9px] font-mono text-zinc-400">Chọn Task Tiền đề</label>
                      ...
                      <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white text-xs px-3 py-1 rounded flex items-center gap-1 font-mono shrink-0 h-[28px]"><Plus size={12}/> Add</button>
                    </form>
                  )}
              ...
              {canManageAssignees ? (
                <>
                  <div className="relative mb-3">
                    ...
                    <input 
                      ...
                      placeholder="Search tên Thành viên..." 
                  ...
                  <div className="space-y-1.5 max-h-44 overflow-y-auto custom-scrollbar pr-1">
                    {isLoadingMembers ? (
                      ...
                      <p className="text-zinc-500 text-[11px] font-mono italic text-center py-2">{searchQuery ? "Không có kết quả" : "Chưa có thành viên nào khác."}</p>
                    ) : (
                      filteredMembers.map((m) => {
                        if (m.role?.roleName === 'Owner') return null; 
                ...
                <div className="space-y-1.5 max-h-44 overflow-y-auto custom-scrollbar pr-1">
                  {assignedMembers.length === 0 ? (
                    <p className="text-zinc-500 text-[11px] font-mono italic text-center py-2">Chưa có thành viên nào được phân công.</p>
                  ) : (
                    assignedMembers.map((m) => (
                      <div key={m.workspaceMemberId || m.memberId || m.id} className="flex flex-col gap-0.5 p-2 bg-white/[0.02] border border-white/5 rounded-lg text-xs">
                        <span className="text-zinc-200 font-medium truncate">{m.fullName || m.memberName || "Ẩn danh"}</span>
