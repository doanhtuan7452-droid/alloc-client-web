...
import { Sparkles, BrainCircuit, History, Send, MessageSquare } from "lucide-react";
import { askAI, fetchProjectAIInsights, fetchProjects } from "../../services/mockApi";
...
export default function AIInsights() {
  ...
  const workspaceId = 12; // Mặc định cho demo

  useEffect(() => {
    fetchProjects(workspaceId).then(list => {
  ...
  const loadHistory = async (pId) => {
    ...
    try {
      const res = await fetchProjectAIInsights(pId);
  ...
  const getTypeIcon = (type) => {
    switch (type) {
      case "Risk Warning": return <Sparkles className="w-4 h-4 text-purple-400" />;
      case "Resource Optimization": return <BrainCircuit className="w-4 h-4 text-emerald-400" />;
      case "Budget Forecast": return <History className="w-4 h-4 text-amber-400" />;
  ...
  const getCardStyle = (type) => {
    switch (type) {
      case "Risk Warning": return "border-purple-800/30 bg-purple-950/10";
      case "Resource Optimization": return "border-emerald-800/30 bg-emerald-950/10";
      case "Budget Forecast": return "border-amber-800/30 bg-amber-950/10";
      default: return "border-blue-800/30 bg-blue-950/10";
    }
  };
      ...
      <div className="w-full max-w-4xl pb-10 flex flex-col flex-grow">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-content-muted text-sm max-w-xl mx-auto">
            Hệ thống AI phân tích dữ liệu chuyên sâu để nhận diện rủi ro, dự phòng ngân sách và tối ưu hóa nguồn lực dự án.
          </p>
        </div>
        ...
        <div className="bg-zinc-900 border border-white/10 rounded-xl p-5 mb-6 z-10 shrink-0">
          <form onSubmit={handleAskAI} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-mono text-zinc-400 mb-1.5">Chọn dự án phân tích</label>
                <select 
              ...
              <div>
                <label className="block text-xs font-mono text-zinc-400 mb-1.5">Loại phân tích</label>
                <select 
                  className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all font-medium"
                  value={analysisType}
                  onChange={(e) => setAnalysisType(e.target.value)}
                >
                  <option value="Risk Warning">Cảnh báo Trễ tiến độ / Rủi ro</option>
                  <option value="Resource Optimization">Tối ưu Nguồn lực / Phân bổ</option>
        ...
        <div className="flex-grow flex flex-col">
          <div className="flex items-center gap-2 mb-4 px-2">
            ...
            <h3 className="text-sm font-bold text-zinc-300">Lịch sử phân tích</h3>
          </div>

          <div className="space-y-4 pb-4">
            {isLoadingHistory ? (
              <div className="text-center py-10 text-zinc-500 text-sm">Đang tải lịch sử...</div>
              ...
              insightsHistory.map(item => (
                <div key={item.logId} className={`p-5 rounded-xl border ${getCardStyle(item.suggestionType)} transition-all animate-fadeIn`}>
                  <div className="flex justify-between items-start mb-3 border-b border-white/5 pb-2">
                    <div className="flex items-center gap-2 font-mono text-sm font-bold">
                      {getTypeIcon(item.suggestionType)}
                      <span className="text-white bg-clip-text whitespace-nowrap">{item.suggestionType}</span>
                    </div>
